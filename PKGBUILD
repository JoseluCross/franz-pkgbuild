...
package() {
  ...
  # Program
	cp -r "$srcdir/locales/"*"" "$pkgdir/opt/franz/locales/"
	cp -r "$srcdir/resources/"*"" "$pkgdir/opt/franz/resources/"
	cp -r "$srcdir/content_shell.pak" "$pkgdir/opt/franz/"
	cp -r "$srcdir/Franz" "$pkgdir/opt/franz/"
	cp -r "$srcdir/icudtl.dat" "$pkgdir/opt/franz/"
	cp -r "$srcdir/libffmpeg.so" "$pkgdir/opt/franz/"
	cp -r "$srcdir/libgcrypt.so.11" "$pkgdir/opt/franz/"
	cp -r "$srcdir/libnode.so" "$pkgdir/opt/franz/"
	cp -r "$srcdir/natives_blob.bin" "$pkgdir/opt/franz/"
	cp -r "$srcdir/snapshot_blob.bin" "$pkgdir/opt/franz/"
